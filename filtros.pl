...
% Base de datos de acompa�amientos
...
acompanamiento(pur�_de_papas, carbohidratos, almuerzo).
...
% Base de datos de calor�as por platillo
calorias(agua, 0).
...
% Regla para encontrar combinaciones que coincidan con las preferencias del cliente y mostrar los nombres con saltos de l�nea.
...
%        writeln(Platillo), % Imprimir la combinaci�n
%        nl % Salto de l�nea
%    ), _).
...
combinacion_cliente(TipoBebida, TipoProteina, TipoAcompanamiento, TipoPostre, MomentoDelDia, CaloriasMinimas) :-
    findall(Platillo, (
        ...
        format(atom(Platillo), 'Bebida: ~w, Proteina: ~w, Acompanamientos: ~w, Postre: ~w, Calor�as: ~w', [Bebida, Proteina, AcompanamientosUnicos, Postre, TotalCalorias]),
        writeln(Platillo), % Imprimir la combinaci�n
        nl % Salto de l�nea
    ), _).
